...
export function DropdownMenuComponent({ items }: DropdownMenuitems) {
  return (
    <DropdownMenu dir="rtl">
      <DropdownMenuTrigger asChild>
        <button className="py-3 text-main-foreground hover:text-white transition-colors duration-300 flex items-center gap-3">
          <ChevronDownIcon
            className={"relative size-6 open:rotate-180"}
            aria-hidden="true"
          />
          {items[0].name}
          {items[0].shortcut && (
            <img
              src={items[0].shortcut}
              alt={`${items[0].name} flag`}
              className="h-3"
            />
          )}
        </button>
